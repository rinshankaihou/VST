...
(** This file is adapted from the CompCert compiler,
    where the memory is instrumented with a freelist.
    Allocation takes block from freelist, 
    instead of [nextblock] as in original CompCert.
 *)
...
(* a modified memory interface, with freelist for allocation *)
Require Import Memperm.
Require Export FMemtype.
...
Record mem' : Type := mkmem {
  ...
  (** original CompCert use [nextblock] in defining [alloc] and [valid]. 
      Here we replace [nextblock] with [freelist] and [nextblockid] for local allocation, 
      and [validblocks] recording valid blocks globally. 
      [validblocks] is defined as a list, to make it finite *)
  validblocks: list block;

  freelist: MemAux.freelist;
  nextblockid: nat;
  
  (** well defined conditions *)
  (** - [freelist_wd] : freelist has no duplicated blocks *)
  freelist_wd:
    forall n n', n <> n' -> get_block freelist n <> get_block freelist n';
  (** - [valid_wd] : [freelist] and [validblocks] are consistent*)
  valid_wd:
    forall n b,
      get_block freelist n = b ->
      (In b validblocks <-> (n < nextblockid)%nat);
(*    forall n b, (n < nextblockid)%nat -> Str_nth n freelist = b -> validblocks b = true; *)
    
  (** Original CompCert memory constraints:
...
Lemma mkmem_ext:
 forall cont1 cont2 acc1 acc2 valid1 valid2 fl1 fl2 next1 next2 a1 a2 b1 b2 c1 c2 d1 d2 e1 e2,
  cont1=cont2 -> acc1=acc2 -> valid1 = valid2 -> fl1 = fl2 -> next1=next2 ->  
  mkmem cont1 acc1 valid1 fl1 next1 a1 b1 c1 d1 e1 =
  mkmem cont2 acc2 valid1 fl1 next2 a2 b2 c2 d2 e2.
Proof.
  intros. subst. f_equal; apply proof_irr.
Qed.

(* ADD *)
(** trivial facts about additional properties *)
Lemma valid_in:
  forall m n b,
    get_block m.(freelist) n = b ->
    (n < m.(nextblockid))%nat ->
    In b m.(validblocks).
Proof.
  intros. rewrite valid_wd; eauto.
Qed.
  
Lemma invalid_out:
  forall m n b,
    Str_nth n m.(freelist) = b ->
    ~ (n < m.(nextblockid))%nat ->
    ~ In b m.(validblocks).
Proof.
  intros. rewrite valid_wd; eauto.
...
Lemma freelist_unique:
  forall n1 n2 m,
    Str_nth n1 m.(freelist) = Str_nth n2 m.(freelist) <->
    n1 = n2.
Proof.
  split; intros; [|subst; auto].
  destruct (Nat.eq_dec n1 n2); auto.
  contradiction (freelist_wd m _ _ n).
Qed.

(** * The freelist and nextblock interface *)  

Definition in_freelist (m: mem) (b: block) : Prop := in_fl m.(freelist) b.

Definition nextblock (m: mem) : block := get_block m.(freelist) m.(nextblockid).

Theorem nextblock_in_freelist:
  forall m, in_freelist m (nextblock m).
Proof.
  unfold in_freelist, nextblock; intros.
  eexists; unfold_fl; eauto.
Qed.
...
Definition valid_block (m: mem) (b: block) := In b (validblocks m).
Definition valid_block_list (m: mem) := validblocks m.

Theorem valid_block_dec: forall (m: mem) (b: block), {valid_block m b} + {~valid_block m b}.
Proof.
  unfold valid_block; intros.
  apply In_dec. exact eq_block.
Qed.

Theorem nextblock_invalid: forall (m: mem), ~ valid_block m (nextblock m).
Proof.
  intros m Contra.
  unfold nextblock, valid_block in Contra.
  eapply invalid_out; eauto.
  Lia.lia.
Qed.

(* a corresponding bool valued valid_block *)
Definition valid_block_bool (m: mem) (b: block) : bool :=
  if (valid_block_dec m b) then true else false.
...
Local Hint Resolve
     freelist_unique
     nextblock_in_freelist
     nextblock_invalid
     valid_not_valid_diff: mem.
(* END EDIT *)
...
Proof.
  ...
  apply invalid_noaccess. unfold valid_block in n. assumption.
  rewrite H0 in H.
  contradiction.
Qed.
...
Proof.
  ...
  assert (perm m b ofs Cur Nonempty).
    apply H. generalize (size_chunk_pos chunk). Lia.lia.
  eauto with mem.
Qed.
...
Lemma valid_access_perm:
  ...
  perm m b ofs k p.
Proof.
  intros. destruct H. apply perm_cur. apply H. generalize (size_chunk_pos chunk). Lia.lia.
Qed.
...
Proof.
  ...
  split; intros.
  split. simpl; red; intros. replace ofs0 with ofs by Lia.lia. auto.
  simpl. apply Zone_divide.
  destruct H. apply H. simpl. Lia.lia.
Qed.
...
Program Definition alloc (m: mem) (lo hi: Z) :=
  (mkmem (PMap.set (nextblock m)
                   (ZMap.init Undef)
                   m.(mem_contents))
         (PMap.set (nextblock m)
                   (fun ofs k => if zle lo ofs && zlt ofs hi then Some Freeable else None)
                   m.(mem_access))
         ((nextblock m) :: m.(validblocks))
         (m.(freelist))
         (m.(nextblockid)+1)%nat
         _ _ _ _ _,
   (nextblock m)).
Next Obligation. apply freelist_wd. assumption. Qed.
Next Obligation.
  unfold nextblock; unfold_fl.
  rewrite freelist_unique, valid_wd; unfold_fl; eauto. Lia.lia.
Qed.
Next Obligation.    
...
Next Obligation.
  ...
  subst b. elim H. auto.
  apply invalid_noaccess. red; intros; elim H. auto.
Qed.
Next Obligation.
...
Program Definition unchecked_free (m: mem) (b: block) (lo hi: Z): mem :=
  mkmem m.(mem_contents)
        ...
        (m.(validblocks))
        (m.(freelist))
        (m.(nextblockid)) _ _ _ _ _.
Next Obligation. apply freelist_wd. auto. Qed.
Next Obligation. apply valid_wd. auto. Qed.
Next Obligation.
...
Next Obligation.
  ...
  destruct (zle lo ofs && zlt ofs hi). auto. apply invalid_noaccess; auto.
  apply invalid_noaccess; auto.
Qed.
Next Obligation.
...
Proof.
  ...
  apply IHvl. intros. apply H. Lia.lia.
  apply ZMap.gso. apply not_eq_sym. apply H. Lia.lia.
Qed.
...
Proof.
  ...
  auto.
  decEq.
  rewrite setN_outside. apply ZMap.gss. Lia.lia.
  apply IHvl.
Qed.
...
Proof.
  ...
  apply H. Lia.lia. apply IHn. intros. apply H. Lia.lia.
Qed.
...
Next Obligation. apply invalid_noaccess; auto. Qed.
Next Obligation.
...
Qed.
Next Obligation.
  specialize (invalid_noaccess m b0 ofs k H). intros.
...
Proof.
  ...
  generalize (size_chunk_pos chunk); Lia.lia.
Qed.
...
Proof.
  ...
  replace (p + Z.succ (Z.of_nat n1)) with ((p + 1) + Z_of_nat n1) by Lia.lia.
  auto.
Qed.
...
Proof.
  ...
  congruence.
  red; intros.
  assert (ofs0 < ofs + n1 \/ ofs0 >= ofs + n1) by Lia.lia.
  destruct H4. apply r; Lia.lia. apply r0; Lia.lia.
Qed.
...
Proof.
  ...
  f_equal.
  rewrite inj_S in H.
  replace ofs with (ofs+0) by Lia.lia.
  apply H; Lia.lia.
  apply IHn.
  intros.
  rewrite <- Zplus_assoc.
  apply H.
  rewrite inj_S. Lia.lia.
Qed.
...
Proof.
  ...
  exploit loadbytes_split. eexact LB. Lia.lia. Lia.lia.
  ...
  simpl. apply Zdivide_plus_r. apply Z.divide_trans with 8; auto. exists 2; auto. exists 1; auto.
  intros L2.
...
Proof.
  ...
  Lia.lia. apply Ptrofs.unsigned_range. auto.
  ...
  unfold Ptrofs.max_unsigned. Lia.lia.
Qed.
...
(* ADD : additional lemmas about freelist, validblocks, etc.. *)
Lemma store_validblocks: validblocks m2 = validblocks m1.
Proof. unfold store in STORE. destruct valid_access_dec; inv STORE. auto. Qed.

Lemma store_freelist: freelist m2 = freelist m1.
Proof. unfold store in STORE. destruct valid_access_dec; inv STORE. auto. Qed.

Lemma in_freelist_store: forall b', in_freelist m1 b' <-> in_freelist m2 b'.
Proof. unfold in_freelist. rewrite store_freelist. tauto. Qed.

Lemma store_nextblockid: nextblockid m2 = nextblockid m1.
Proof. unfold store in STORE. destruct valid_access_dec; inv STORE. auto. Qed.
(* END ADD *)
...
Proof.
  unfold valid_block; intros. rewrite store_validblocks; auto.
Qed.

Theorem store_valid_block_2:
  forall b', valid_block m2 b' -> valid_block m1 b'.
Proof.
  unfold valid_block; intros. rewrite store_validblocks in H; auto.
Qed.
...
Theorem load_store_same:
  load chunk m2 b ofs = Some (Val.load_result chunk v).
Proof.
  apply load_store_similar_2; auto. Lia.lia.
Qed.
...
Proof.
  ...
  auto with coqlib. Lia.lia.
  right. apply IHvl. Lia.lia.
Qed.
...
- subst ofs'. left; split. auto. unfold c'. simpl.
  rewrite setN_outside by Lia.lia. apply ZMap.gss.
...
Proof.
  ...
  f_equal. eapply mkmem_ext; eauto. congruence.
  elim n. apply valid_access_compat with chunk1; auto. Lia.lia.
  elim n. apply valid_access_compat with chunk2; auto. Lia.lia.
Qed.
...
Proof.
  ...
  elim n. apply valid_access_compat with Mfloat64; auto. simpl; Lia.lia.
Qed.
...
Proof.
  ...
  destruct (valid_access_dec m1 chunk b ofs Writable).
  f_equal. eapply mkmem_ext; eauto.
...
(* ADD : additional lemmas about freelist, validblocks, etc.. *)
Lemma storebytes_freelist:
  freelist m2 = freelist m1.
Proof. unfold storebytes in STORE. destruct range_perm_dec; inv STORE; auto. Qed.

Lemma in_freelist_storebytes:
  forall b', in_freelist m1 b' <-> in_freelist m2 b'.
Proof. unfold in_freelist; rewrite storebytes_freelist; tauto. Qed.

Lemma storebytes_validblocks:
  validblocks m2 = validblocks m1.
Proof. unfold storebytes in STORE. destruct range_perm_dec; inv STORE; auto. Qed.

Lemma storebytes_nextblockid:
  nextblockid m2 = nextblockid m1.
Proof. unfold storebytes in STORE. destruct range_perm_dec; inv STORE; auto. Qed.
(* END ADD *)
...
Theorem storebytes_valid_block_1:
  forall b', valid_block m1 b' -> valid_block m2 b'.
Proof.
  unfold valid_block; intros. rewrite storebytes_validblocks; auto.
Qed.

Theorem storebytes_valid_block_2:
  forall b', valid_block m2 b' -> valid_block m1 b'.
Proof.
  unfold valid_block; intros. rewrite storebytes_validblocks in H; auto.
Qed.
...
Proof.
  ...
  simpl. decEq. Lia.lia.
  simpl length. rewrite inj_S. simpl. rewrite IHbytes1. decEq. Lia.lia.
Qed.
...
Proof.
  ...
  inv ST1; inv ST2; simpl. decEq. eapply mkmem_ext; auto.
  ...
  apply r. Lia.lia.
  eapply perm_storebytes_2; eauto. apply r0. Lia.lia.
Qed.
...
Proof.
  ...
  eexact H. instantiate (1 := ofs0). rewrite app_length. rewrite inj_plus. Lia.lia.
  auto.
...
(* ADD : ?? *)
Lemma alloc_freelist: freelist m2 = freelist m1.
Proof. injection ALLOC. intros. subst. auto. Qed.

Lemma in_freelist_alloc:
  forall b', in_freelist m1 b' <-> in_freelist m2 b'.
Proof. unfold in_freelist. erewrite alloc_freelist; eauto. tauto. Qed.

Lemma alloc_validblocks: validblocks m2 = nextblock m1 :: validblocks m1.
Proof. injection ALLOC. intros. subst. auto. Qed.

Lemma alloc_nextblockid: nextblockid m2 = S (nextblockid m1).
Proof. injection ALLOC; intros. subst. simpl. Lia.lia. Qed.
...
Theorem valid_block_alloc:
  forall b', valid_block m1 b' -> valid_block m2 b'.
Proof.
  unfold valid_block; intros. rewrite alloc_validblocks.
  simpl. auto.
Qed.
...
Theorem fresh_block_alloc:
  ~(valid_block m1 b).
Proof.
  rewrite alloc_result. apply nextblock_invalid.
Qed.

Theorem valid_new_block:
  valid_block m2 b.
Proof.
  unfold valid_block. rewrite alloc_result, alloc_validblocks. simpl; auto.
Qed.
...
Proof.
  unfold valid_block; intros.
  rewrite alloc_validblocks in H. rewrite alloc_result.
  destruct H; auto.
Qed.
...
Proof.
  ...
  rewrite invalid_noaccess in H. contradiction. subst b'. apply nextblock_invalid.
Qed.
...
Proof.
  ...
  rewrite zlt_true. simpl. auto with mem. Lia.lia. Lia.lia.
Qed.
...
Proof.
  ...
  red; intros. apply perm_alloc_2. Lia.lia.
Qed.
...
Proof.
  ...
  assert (perm m2 b ofs Cur p). apply H0. Lia.lia.
  assert (perm m2 b (ofs + size_chunk chunk - 1) Cur p). apply H0. Lia.lia.
...
Proof.
  intros. assert (exists v, load chunk m2 b ofs = Some v).
    ...
    red; intros. eapply perm_implies. apply perm_alloc_2. Lia.lia. auto with mem.
...
(* ADD: *)
Lemma free_freelist: freelist m2 = freelist m1.
Proof. unfold free in FREE. destruct range_perm_dec; inversion FREE; auto. Qed.

Lemma in_freelist_free: forall b', in_freelist m1 b' <-> in_freelist m2 b'.
Proof. unfold in_freelist; rewrite free_freelist; tauto. Qed.

Lemma free_validblocks: validblocks m2 = validblocks m1.
Proof. unfold free in FREE. destruct range_perm_dec; inversion FREE; auto. Qed.

Lemma free_nextblockid: nextblockid m2 = nextblockid m1.
Proof. unfold free in FREE. destruct range_perm_dec; inversion FREE; auto. Qed.
(* END ADD *)
...
Proof.
  ...
  destruct (zlt lo hi). intuition. right. Lia.lia.
Qed.
...
Proof.
  ...
  elim (valid_access_free_2 chunk ofs p); auto. Lia.lia.
Qed.
...
Proof.
  ...
  eapply perm_free_1; eauto. destruct H; auto. right; Lia.lia.
Qed.
...
(* ADD : *)
Lemma drop_freelist: freelist m' = freelist m.
Proof. unfold drop_perm in DROP. destruct range_perm_dec; inv DROP; auto. Qed.

Lemma in_freelist_drop: forall b', in_freelist m b' <-> in_freelist m' b'.
Proof. unfold in_freelist; rewrite drop_freelist; tauto. Qed.

Lemma drop_validblocks: validblocks m' = validblocks m.
Proof. unfold drop_perm in DROP. destruct range_perm_dec; inv DROP; auto. Qed.

Lemma drop_nextblockid: nextblockid m' = nextblockid m.
Proof. unfold drop_perm in DROP. destruct range_perm_dec; inv DROP; auto. Qed.
(* END ADD *)
...
Proof.
  unfold valid_block; rewrite drop_validblocks; auto.
Qed.
...
Proof.
  ...
  byContradiction. intuition Lia.lia.
  auto. auto. auto.
Qed.
...
Proof.
  ...
  apply perm_implies with p. eapply perm_drop_1; eauto. Lia.lia.
...
Proof.
  ...
  destruct (zlt ofs0 lo). eapply perm_drop_3; eauto.
  destruct (zle hi ofs0). eapply perm_drop_3; eauto.
  apply perm_implies with p. eapply perm_drop_1; eauto. Lia.lia. intuition.
  eapply perm_drop_3; eauto.
...
Proof.
  ...
  eapply perm_inj; eauto. apply H0. Lia.lia.
Qed.
...
Proof.
  ...
  replace (ofs + delta + size_chunk chunk)
     with ((ofs + size_chunk chunk) + delta) by Lia.lia.
...
Proof.
  ...
  eapply range_perm_cur in H1. eapply H1. Lia.lia.
...
Proof.
  ...
  replace (ofs + delta + len) with ((ofs + len) + delta) by Lia.lia.
  eapply range_perm_inj; eauto with mem.
  apply getN_inj; auto.
  destruct (zle 0 len). rewrite Z2Nat.id; auto.
  rewrite Z_to_nat_neg. simpl. red; intros; lia. Lia.lia.
Qed.
...
Proof.
  ...
  auto.
  replace (p + delta + 1) with ((p + 1) + delta) by Lia.lia.
  apply IHlist_forall2; auto.
  intros. rewrite ZMap.gsspec at 1. destruct (ZIndexed.eq q0 p). subst q0.
  rewrite ZMap.gss. auto.
  rewrite ZMap.gso. auto. unfold ZIndexed.t in *. Lia.lia.
Qed.
...
(* mem_contents *)
  ...
  assert (b2 <> b2 \/ ofs0 + delta0 <> (r - delta) + delta).
    ...
    eapply perm_implies. apply perm_cur_max. apply A. Lia.lia. auto with mem.
  destruct H8. congruence. Lia.lia.
  (* block <> b1, block <> b2 *)
...
(* mem_contents *)
  ...
  destruct (zle (ofs + size_chunk chunk) (ofs0 + delta)). Lia.lia.
  byContradiction. eapply H0; eauto. Lia.lia.
  eauto with mem.
Qed.
...
Proof.
  ...
  assert (range_perm m2 b2 (ofs + delta) (ofs + delta + Z_of_nat (length bytes2)) Cur Writable).
    ...
    rewrite (list_forall2_length H3). Lia.lia.
...
(* mem_contents *)
  ...
  assert (b2 <> b2 \/ ofs0 + delta0 <> (r - delta) + delta).
    ...
    rewrite (list_forall2_length H3). Lia.lia.
    eauto 6 with mem.
  destruct H9. congruence. Lia.lia.
  (* block <> b1, block <> b2 *)
  eauto.
Qed.
...
(* mem_contents *)
  ...
  rewrite setN_outside. auto.
  destruct (zlt (ofs0 + delta) ofs); auto.
  destruct (zle (ofs + Z_of_nat (length bytes2)) (ofs0 + delta)). Lia.lia.
  byContradiction. eapply H0; eauto. Lia.lia.
  eauto with mem.
Qed.
...
(* align *)
  ...
  assert (lo <= ofs < hi).
  { eapply perm_alloc_3; eauto. apply H6. generalize (size_chunk_pos chunk); Lia.lia. }
...
Proof.
  ...
  replace ofs with ((ofs - delta) + delta) by Lia.lia.
  eapply perm_inj; eauto. eapply range_perm_drop_1; eauto. Lia.lia.
...
(* perm *)
  ...
  assert (perm_order p p0).
    eapply perm_drop_2.  eexact H0. instantiate (1 := ofs). Lia.lia. eauto.
  apply perm_implies with p; auto.
  eapply perm_drop_1. eauto. Lia.lia.
  (* b1 <> b0 *)
  eapply perm_drop_3; eauto.
  ...
  eapply range_perm_drop_1; eauto. Lia.lia. auto with mem.
...
Proof.
  ...
  byContradiction. exploit H1; eauto. Lia.lia.
  (* align *)
  eapply mi_align0; eauto.
  (* contents *)
  intros.
...
(** COMMENT: [extends'] here is too strong to be used in our LDSim,
    because it requires equal on freelist and validblock set, which is 
    possibly not equal after a rely step in simulation *)
Record extends' (m1 m2: mem) : Prop :=
  mk_extends {
      (* EDIT *)
      mext_validblocks:
        forall b, In b (validblocks m1) <-> In b (validblocks m2);
      mext_freelist: freelist m1 = freelist m2;
      mext_nextblockid: nextblockid m1 = nextblockid m2;
      (* END EDIT *)
...
Proof.
  intros. constructor; intros; auto. split; auto. constructor.
  intros. unfold inject_id in H; inv H. replace (ofs + 0) with ofs by Lia.lia. auto.
  intros. unfold inject_id in H; inv H. apply Z.divide_0_r.
  intros. unfold inject_id in H; inv H. replace (ofs + 0) with ofs by Lia.lia.
...
Proof.
  ...
  replace (ofs + 0) with ofs in A by Lia.lia. auto.
...
Proof.
  intros. inv H.
  replace ofs with (ofs + 0) by Lia.lia. eapply loadbytes_inj; eauto.
Qed.
...
Proof.
  ...
  replace (ofs + 0) with ofs in A by Lia.lia. auto.
  constructor; auto.
  rewrite (store_validblocks _ _ _ _ _ _ H0).
  rewrite (store_validblocks _ _ _ _ _ _ A).
  auto.
  rewrite (store_freelist _ _ _ _ _ _ H0).
  rewrite (store_freelist _ _ _ _ _ _ A).
  auto.
  rewrite (store_nextblockid _ _ _ _ _ _ H0).
  rewrite (store_nextblockid _ _ _ _ _ _ A).
  auto.
...
Proof.
  intros. inversion H. constructor.
  rewrite (store_validblocks _ _ _ _ _ _ H0). auto.
  rewrite (store_freelist _ _ _ _ _ _ H0). auto.
  rewrite (store_nextblockid _ _ _ _ _ _ H0). auto.  
  eapply store_outside_inj; eauto.
  unfold inject_id; intros. inv H2. eapply H1; eauto. Lia.lia.
...
Proof.
  ...
  replace (ofs + 0) with ofs in A by Lia.lia. auto.
  constructor; auto.
  rewrite (storebytes_validblocks _ _ _ _ _ H0).
  rewrite (storebytes_validblocks _ _ _ _ _ A).
  auto.
  rewrite (storebytes_freelist _ _ _ _ _ H0).
  rewrite (storebytes_freelist _ _ _ _ _ A).
  auto.
  rewrite (storebytes_nextblockid _ _ _ _ _ H0).
  rewrite (storebytes_nextblockid _ _ _ _ _ A).
  auto.
...
Theorem storebytes_outside_extends:
  ...
  extends m1 m2 ->
  storebytes m2 b ofs bytes2 = Some m2' ->
  (forall ofs', perm m1 b ofs' Max Readable -> ofs <= ofs' < ofs + Z_of_nat (length bytes2) -> False) ->
  extends m1 m2'.
Proof.
  intros. inversion H. constructor.
  rewrite (storebytes_validblocks _ _ _ _ _ H0). auto.
  rewrite (storebytes_freelist _ _ _ _ _ H0). auto.
  rewrite (storebytes_nextblockid _ _ _ _ _ H0). auto.  
...
Proof.
  ...
  constructor; intros.
  rewrite (alloc_validblocks _ _ _ _ _ H0).
  rewrite (alloc_validblocks _ _ _ _ _ ALLOC).
  unfold nextblock. simpl.
  rewrite mext_freelist0, mext_nextblockid0, mext_validblocks0. tauto.
  rewrite (alloc_freelist _ _ _ _ _ H0).
  rewrite (alloc_freelist _ _ _ _ _ ALLOC).
  auto.
  rewrite (alloc_nextblockid _ _ _ _ _ H0).
  rewrite (alloc_nextblockid _ _ _ _ _ ALLOC).
  auto.
  ...
  subst b0. 
  assert (EITHER: lo1 <= ofs < hi1 \/ ~(lo1 <= ofs < hi1)) by Lia.lia.
  destruct EITHER.
...
Proof.
  intros. inv H. constructor.
  rewrite (free_validblocks _ _ _ _ _ H0). auto.
  rewrite (free_freelist _ _ _ _ _ H0). auto.
  rewrite (free_nextblockid _ _ _ _ _ H0). auto.
...
Proof.
  ...
  unfold inject_id; intros. inv H. eapply H1; eauto. Lia.lia.
...
Proof.
  ...
  constructor.
  intros.
  rewrite (free_validblocks _ _ _ _ _ H0).
  rewrite (free_validblocks _ _ _ _ _ FREE). auto.
  rewrite (free_freelist _ _ _ _ _ H0). 
  rewrite (free_freelist _ _ _ _ _ FREE). auto.  
  rewrite (free_nextblockid _ _ _ _ _ H0).
  rewrite (free_nextblockid _ _ _ _ _ FREE). auto.
  eapply free_right_inj with (m1 := m1'); eauto.
  eapply free_left_inj; eauto.
  unfold inject_id; intros. inv H1.
  eapply perm_free_2. eexact H0. instantiate (1 := ofs); Lia.lia. eauto.
...
Proof.
  intros. inv H. unfold valid_block. rewrite mext_validblocks0. tauto.
Qed.
...
Proof.
  intros. inv H. replace ofs with (ofs + 0) by Lia.lia.
...
Theorem in_freelist_extends:
  forall m1 m2, extends m1 m2 ->
           (forall b, in_freelist m1 b <-> in_freelist m2 b).
Proof.
  intros. inv H.
  unfold in_freelist. rewrite mext_freelist0. tauto.
Qed.
...
Record inject' (f: meminj) (m1 m2: mem) : Prop :=
  mk_inject {
    ...
    (* ADD *)
    (* TODO: should we require freelist injected? *)
    mi_freelist:
      forall b b' delta,
        in_freelist m1 b ->
        f b = Some (b', delta) ->
...
Theorem valid_block_inject_1:
  ...
  valid_block m1 b1.
Proof.
  intros. destruct (valid_block_dec m1 b1); auto.
  exfalso. eapply mi_freeblocks in n; eauto. congruence.
Qed.
...
Proof.
  ...
  assert (0 <= delta <= Ptrofs.max_unsigned).
    generalize (Ptrofs.unsigned_range ofs1). Lia.lia.
  unfold Ptrofs.add. repeat rewrite Ptrofs.unsigned_repr; Lia.lia.
Qed.
...
Proof.
  ...
  apply H0. generalize (size_chunk_pos chunk). Lia.lia.
Qed.
...
Proof.
  ...
  intros [A B].
  pose proof (Ptrofs.unsigned_range ofs).
  unfold Ptrofs.add. repeat rewrite Ptrofs.unsigned_repr; auto; Lia.lia.
Qed.
...
Proof.
  ...
  apply perm_cur_max. apply (H5 (Ptrofs.unsigned ofs1)). Lia.lia.
  apply perm_cur_max. apply (H1 (Ptrofs.unsigned ofs2)). Lia.lia.
Qed.
...
Proof.
  ...
  destruct H5. congruence. right. destruct H5. left; congruence. right. Lia.lia.
  ...
  apply Intv.range_disjoint'; simpl; try Lia.lia.
  unfold Intv.disjoint, Intv.In; simpl; intros. red; intros.
  exploit mi_no_overlap; eauto.
  instantiate (1 := x - delta1). apply H2. Lia.lia.
  instantiate (1 := x - delta2). apply H3. Lia.lia.
  intuition.
Qed.
...
Proof.
  intros.
  assert (P: al > 0) by Lia.lia.
  assert (Q: Z.abs al <= Z.abs sz). apply Zdivide_bounds; auto. Lia.lia.
  rewrite Z.abs_eq in Q; try Lia.lia. rewrite Z.abs_eq in Q; try Lia.lia.
  ...
  assert (valid_access m chunk b ofs Nonempty).
    split. red; intros; apply H3. Lia.lia. congruence.
...
(* freeblocks *)
  eauto with mem.
(* mappedblocks *)
  intros. apply mi_mappedblocks0 in H3. eapply store_valid_block_1; eauto.
(* freelist *)
  intros. apply mi_freelist0 in H4; unfold in_freelist.
  erewrite store_freelist; eauto.
  erewrite <- store_freelist; eauto.
(* no overlap *)
  red; intros. eauto with mem.
(* representable *)
...
(* mappedblocks *)
  eauto with mem.
(* freelist *)
  intros. apply mi_freelist0 in H3; unfold in_freelist; auto.
  erewrite <- store_freelist; eauto.  
(* no overlap *)
  red; intros. eauto with mem.
(* representable *)
...
(* mappedblocks *)
  eauto with mem.
(* freelist *)
  intros. unfold in_freelist.  erewrite store_freelist; eauto.
  eapply mi_freelist0; eauto.
(* no overlap *)
  auto.
(* representable *)
  eauto with mem.
...
(* freelist *)
  intros. unfold in_freelist. erewrite storebytes_freelist; eauto.
  eapply mi_freelist; eauto. unfold in_freelist.
  erewrite <- storebytes_freelist; eauto.
(* no overlap *)
...
(* mappedblocks *)
  eauto with mem.
(* freelist *)
  intros. unfold in_freelist. eapply mi_freelist; eauto.
  unfold in_freelist. erewrite <- storebytes_freelist; eauto.
(* no overlap *)
  red; intros. eapply mi_no_overlap0; eauto; eapply perm_storebytes_2; eauto.
(* representable *)
  intros. eapply mi_representable0; eauto.
...
(* freelist *)
  unfold in_freelist. erewrite  storebytes_freelist; eauto.
  eapply mi_freelist; eauto. unfold in_freelist. erewrite <- storebytes_freelist; eauto.
(* no overlap *)
  red; intros. eapply mi_no_overlap0; eauto; eapply perm_storebytes_2; eauto.
(* representable *)
  intros. eapply mi_representable0; eauto.
...
(* mappedblocks *)
  eauto with mem.
(* freelist *)
  intros. unfold in_freelist. erewrite alloc_freelist; eauto.
  eapply mi_freelist0; eauto.
(* no overlap *)
  auto.
(* representable *)
...
(* freelist *)
  apply alloc_freelist in H0.
  unfold in_freelist. rewrite H0. unfold f'. intros; destruct eq_block.
  discriminate. eapply mi_freelist0; eauto.
(* no overlap *)
  unfold f'; red; intros.
...
Theorem alloc_left_mapped_inject:
  ...
  alloc m1 lo hi = (m1', b1) ->
  valid_block m2 b2 ->
  (* EDIT : b2 have to be in freelist *)
  in_freelist m2 b2 ->
  ...
  /\ (forall b, b <> b1 -> f' b = f b).
Proof.
  intros until 4. rename H2 into H_in. intros. inversion H. 
  ...
  assert (mem_inj f' m1 m2).
    ...
    unfold f'; intros. destruct (eq_block b0 b1).
      ...
      eapply perm_valid_block with (ofs := ofs). apply H9. generalize (size_chunk_pos chunk); Lia.lia.
      eauto.
    unfold f'; intros. destruct (eq_block b0 b1).
      inversion H8. subst b0 b3 delta0.
      elim (fresh_block_alloc _ _ _ _ _ H0). eauto with mem.
...
(* mappedblocks *)
  unfold f'; intros. destruct (eq_block b b1). congruence. eauto.
(* freelist *)
  apply alloc_freelist in H0. unfold in_freelist; rewrite H0.
  unfold f'; intros. destruct eq_block.
  inv H10. auto.
  eapply mi_freelist0; eauto.
(* overlap *)
  unfold f'; red; intros.
...
(* representable *)
  ...
  destruct (eq_block b b1).
   subst. injection H9; intros; subst b' delta0. destruct H10.
    ...
    generalize (Ptrofs.unsigned_range_2 ofs). Lia.lia.
   exploit perm_alloc_inv; eauto; rewrite dec_eq_true; intro.
   exploit H3. apply H4 with (k := Max) (p := Nonempty); eauto.
   generalize (Ptrofs.unsigned_range_2 ofs). Lia.lia.
  eapply mi_representable0; try eassumption.
...
(* perm inv *)
  ...
  assert (EITHER: lo <= ofs < hi \/ ~(lo <= ofs < hi)) by Lia.lia.
  destruct EITHER.
  left. apply perm_implies with Freeable; auto with mem. eapply perm_alloc_2; eauto.
...
Proof.
  ...
  unfold in_freelist. erewrite alloc_freelist, alloc_result; eauto.
  unfold nextblock. unfold_fl. eauto.
  
  instantiate (1 := 0). unfold Ptrofs.max_unsigned. generalize Ptrofs.modulus_pos; Lia.lia.
  auto.
  intros. apply perm_implies with Freeable; auto with mem.
  eapply perm_alloc_2; eauto. Lia.lia.
  red; intros. apply Zdivide_0.
...
(* mappedblocks *)
  auto.
(* freelist *)
  intros. apply free_freelist in H0. unfold in_freelist in *. rewrite H0 in H1.
  eapply mi_freelist0; eauto.
(* no overlap *)
  red; intros. eauto with mem.
(* representable *)
...
(* mappedblocks *)
  eauto with mem.
(* freelist *)
  intros. apply free_freelist in H0. unfold in_freelist in *. rewrite H0.
  eapply mi_freelist0; eauto.
(* no overlap *)
  auto.
(* representable *)
...
Proof.
  ...
  exists lo, hi; split; auto with coqlib. Lia.lia.
  ...
  eapply free_range_perm; eauto. Lia.lia.
  intros [A|A]. congruence. Lia.lia.
Qed.
...
Proof.
  ...
  intros. unfold valid_block in *. erewrite drop_validblocks; eauto.
  unfold in_freelist. intros. erewrite drop_freelist; eauto.
  eapply mi_freelist0; eauto.
...
Proof.
  ...
  replace (ofs + (delta' + delta'')) with ((ofs + delta') + delta'') by Lia.lia.
  eauto.
  (* align *)
  destruct (f b1) as [[b' delta'] |] eqn:?; try discriminate.
  destruct (f' b') as [[b'' delta''] |] eqn:?; inv H.
  ...
  red; intros. replace ofs0 with ((ofs0 - delta') + delta') by Lia.lia.
  eapply mi_perm0; eauto. apply H0. Lia.lia.
  (* memval *)
  destruct (f b1) as [[b' delta'] |] eqn:?; try discriminate.
  destruct (f' b') as [[b'' delta''] |] eqn:?; inv H.
  replace (ofs + (delta' + delta'')) with ((ofs + delta') + delta'') by Lia.lia.
...
(* mapped *)
  ...
  eauto.
(* freelist *)
  intros. destruct (f b) eqn:? ; [|discriminate].
  destruct p. destruct (f' b0) eqn:? ; [|discriminate]. destruct p. inv H0.
  eapply mi_freelist1; eauto. 
...
(* no overlap *)
  ...
  assert (delta1y = delta2y) by congruence. right; Lia.lia.
...
(* representable *)
  ...
  replace (Ptrofs.unsigned ofs + delta1 - 1) with
    ((Ptrofs.unsigned ofs - 1) + delta1) by Lia.lia.
...
(* perm inv *)
  ...
  replace (ofs + (delta' + delta'')) with ((ofs + delta') + delta'') in H0 by Lia.lia.
...
(* freelist *)
  eapply mi_freelist0; eauto.
  unfold in_freelist in *. congruence.
(* no overlap *)
...
(* inj *)
  ...
  apply extensionality; intros. unfold compose_meminj, inject_id.
  destruct (f x) as [[y delta] | ]; auto. decEq. decEq. Lia.lia.
(* unmapped *)
  eauto.
(* mapped *)
  erewrite <- valid_block_extends; eauto.
(* freelist *)
  eapply mi_freelist0 in H; eauto.
  unfold in_freelist in *; congruence.
(* no overlap *)
...
Proof.
  ...
  (* nextblock *)
  rewrite (mext_validblocks0 b). auto.
  (* freelist *)
  congruence.
  (* nextblockid *)
  congruence.
...
(* EDIT : Here we don't have a concepct of threashold. 
   Instead, we change the threashold to a list of blocks of concern. *)
Definition flat_inj (thr: list block) : meminj :=
  fun (b: block) => if (In_dec eq_block b thr) then Some(b, 0) else None.

Definition inject_neutral (thr: list block) (m: mem) :=
...
Proof.
  ...
  destruct (in_dec eq_block b1 thr); inversion H0; subst.
  destruct (in_dec eq_block b2 thr); inversion H1; subst.
  auto.
Qed.

Theorem neutral_inject:
  forall m, inject_neutral (valid_block_list m) m -> inject (flat_inj (valid_block_list m)) m m.
Proof.
  unfold valid_block_list. intros. constructor.
(* meminj *)
  auto.
(* freeblocks *)
...
(* mappedblocks *)
  unfold flat_inj, valid_block; intros.
  destruct (in_dec eq_block b (validblocks m)); inversion H0; subst. auto.
(* freelist *)
  unfold flat_inj; intros.
  destruct in_dec; inversion H1. subst; auto.
(* no overlap *)
  apply flat_inj_no_overlap.
(* range *)
  unfold flat_inj; intros.
  destruct in_dec; inv H0. generalize (Ptrofs.unsigned_range_2 ofs); Lia.lia.
(* perm inv *)
  unfold flat_inj; intros.
  destruct in_dec; inv H0.
...
(* perm *)
  ...
  replace (ofs + 0) with ofs by Lia.lia; auto.
(* align *)
...
Proof.
  ...
  replace (ofs + 0) with ofs by Lia.lia.
...
Record unchanged_on (m_before m_after: mem) : Prop :=
  mk_unchanged_on {
      (* EDIT *)
      (* Here we change the nextblock_unchanged into validblock unchanged. *)
  unchanged_on_validblock:
    forall b, valid_block m_before b -> valid_block m_after b;
  unchanged_on_perm:
    forall b ofs k p,
...
Proof.
  intros; constructor. auto. tauto. tauto.
Qed.
...
Proof.
  unfold valid_block; intros. eapply unchanged_on_validblock in H; eauto. 
Qed.
...
Proof.
  intros; constructor.
- intros. do 2 (eapply unchanged_on_validblock; eauto).
...
+ unfold loadbytes. destruct H.
  ...
  apply getN_exten. intros. rewrite Z2Nat.id in H by Lia.lia.
...
+ rewrite <- H1. apply loadbytes_unchanged_on_1; auto.
  exploit loadbytes_range_perm; eauto. instantiate (1 := ofs). Lia.lia.
  intros. eauto with mem.
Qed.
...
- unfold valid_block. rewrite (store_validblocks _ _ _ _ _ _ H). auto.
...
Proof.
  intros; constructor; intros.
- unfold valid_block; rewrite (storebytes_validblocks _ _ _ _ _ H). auto.
...
Proof.
  intros; constructor; intros.
- unfold valid_block; rewrite (alloc_validblocks _ _ _ _ _ H). simpl; auto.
- split; intros.
...
Proof.
  intros; constructor; intros.
- unfold valid_block; rewrite (free_validblocks _ _ _ _ _ H). auto.
- split; intros.
  eapply perm_free_1; eauto.
  destruct (eq_block b0 b); auto. destruct (zlt ofs lo); auto. destruct (zle hi ofs); auto.
  subst b0. elim (H0 ofs). Lia.lia. auto.
...
Proof.
  intros; constructor; intros.
- unfold valid_block; rewrite (drop_validblocks _ _ _ _ _ _ H). auto.
...
Require Import GMemory.
Definition strip (fm: mem) : gmem :=
  GMem.mk_gmem
    (Mem.mem_contents fm)
    (Mem.mem_access fm)
    (Mem.validblocks fm)
    (Mem.access_max fm)
    (Mem.invalid_noaccess fm)
    (Mem.contents_default fm).

Inductive embed (gm: gmem) (fl: freelist) (m: mem) : Prop :=
  Embed: Mem.freelist m = fl ->
         strip m = gm ->
         embed gm fl m.
