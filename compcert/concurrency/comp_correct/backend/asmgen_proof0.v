...
Proof.
  ...
  inv H. congruence. replace (pos0 + 1 - 1) with pos0 by Lia.lia.
  eauto.
Qed.
...
Proof.
  ...
  generalize (list_length_z_pos c). Lia.lia.
  rewrite list_length_z_cons. Lia.lia.
Qed.
...
Proof.
  ...
  induction 1; intros; simpl.
  rewrite H. rewrite list_length_z_cons. generalize (list_length_z_pos c'). Lia.lia.
  rewrite list_length_z_cons. generalize (IHcode_tail _ _ H0). Lia.lia.
  eauto.
Qed.
...
Proof.
  ...
  generalize (code_tail_bounds_2 _ _ _ _ H0). Lia.lia.
Qed.
...
Opaque transl_instr.
  ...
  apply transf_function_len in TF. Lia.lia.
...
Proof.
  ...
  generalize (code_tail_no_bigger _ _ _ H3); simpl; intro; Lia.lia.
  generalize (code_tail_no_bigger _ _ _ H3); simpl; intro; Lia.lia.
  f_equal. eauto.
Qed.
...
Proof.
  ...
  replace (pos + 1 - pos) with (0 + 1) by Lia.lia. constructor. constructor.
  rewrite list_length_z_cons. generalize (list_length_z_pos c). Lia.lia.
  intros. generalize (IHc (pos + 1) c' H). intros [pos' [A [B C]]].
  exists pos'. split. auto. split.
  replace (pos' - pos) with ((pos' - (pos + 1)) + 1) by Lia.lia.
  constructor. auto.
  rewrite list_length_z_cons. Lia.lia.
Qed.
