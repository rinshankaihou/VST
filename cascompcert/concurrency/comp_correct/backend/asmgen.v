(* Adapted from CompCert's [x86/Asmgen.v]. *)
...
Require Import Mach_local ASM_local CUAST DisableDebug Asmgen.

(** Since we have changed instruction set of Asm, we need override some definitions related to instruction/code/function *)
...
Definition transf_function (f: Mach.function) : res ASM_local.function :=
  do tf <- transl_function f;
  if zlt Ptrofs.max_unsigned (list_length_z tf.(fn_code))
  then Error (msg "code size exceeded")
  else OK tf.

Definition transf_fundef (f: Mach.fundef) : res ASM_local.fundef :=
  transf_partial_fundef transf_function f.

Definition transf_program (cu: Mach_comp_unit) : res Asm_comp_unit :=
