...
(* Freelist interfaces: in_freelist, freelist_order *)

(* At the local view, freelist is a infinite set *)
(* allocation would always allocate new blocks in freelist *)

Parameter in_freelist: forall (m: mem) (b: block), Prop.

(* freelist is an invariant under memory operations *)

Axiom in_freelist_alloc:
  forall m1 lo hi m2 b,
    alloc m1 lo hi = (m2, b) ->
    (forall b', in_freelist m1 b' <-> in_freelist m2 b').

Axiom in_freelist_free:
  forall m1 b lo hi m2,
    free m1 b lo hi = Some m2 ->
    (forall b', in_freelist m1 b' <-> in_freelist m2 b').

Axiom in_freelist_drop:
  forall m1 b lo hi p m2,
    drop_perm m1 b lo hi p = Some m2 ->
    (forall b', in_freelist m1 b' <-> in_freelist m2 b').

Axiom in_freelist_store:
  forall chunk m1 b ofs v m2,
    store chunk m1 b ofs v = Some m2 ->
    (forall b', in_freelist m1 b' <-> in_freelist m2 b').

Axiom in_freelist_storebytes:
  forall m1 b ofs bytes m2,
    storebytes m1 b ofs bytes = Some m2 ->
    (forall b', in_freelist m1 b' <-> in_freelist m2 b').

(* load returns identical memory thus freelist trivially stays unchanged *)
...
(* [ ADD ] *)
(* relations between nextblock and freelist *)

(* nextblock is in freelist *)
Axiom nextblock_in_freelist:
  forall m, in_freelist m (nextblock m).
(* [ END ADD ] *)


(* [ EDIT ] *)
(* here valid_block is not able to be defined by blockid < nextblock *)
Parameter valid_block: mem -> block -> Prop.
Parameter valid_block_list: mem -> list block.

(* property of valid_block: decidable *)
Axiom valid_block_dec: forall (m: mem) (b: block), {valid_block m b} + {~valid_block m b}.

(* relations between valid_block, nextblock, and freelist *)
(* (1) nextblock is invalid *)
Axiom nextblock_invalid: forall (m: mem), ~ valid_block m (nextblock m).
...
(* [ REMOVED ] we do not define empty memory *)
(*
...
to describe the relation between the new nextblock and valid_block,
freelist, etc. *)
...
(* [ ADD ] *)
(* relation between extends and freelist:
   we assume that freelist is preserved by memory extension.
   TODO: WHY?
 *)
Axiom in_freelist_extends:
...
(* [ ADD ] *)
(* relation between injection and freelist: *)
(* TODO: WHY? *)
(* (1) we assume that freelist is preserved by memory injection *)
(* 
Axiom freelist_inject_1:
  forall f m1 m2,
    inject f m1 m2 ->
    (forall b, in_freelist m1 b <-> in_freelist m2 b).
*)
(* (2) blocks in freelist of m1 would be injected to blocks in
       freelist of m2 *)
Axiom freelist_inject:
  forall f m1 m2,
    inject f m1 m2 ->
    forall b b' delta,
      in_freelist m1 b ->
...
(* [ EDIT ] *)
(* we require that the extended range of injection (b2, delta) should
   be in freelist of m2 *)
...
(* [ EDIT ] *)
(* because there is no orders on block now, we have to change the
   threashold from a block id to a block set *)
(* here we model block set as a mapping from block to bool, since we
   need it decidable *)

Definition flat_inj (thr: list block) : meminj :=
  fun (b: block) => if (In_dec eq_block b thr) then Some(b, 0) else None.

Parameter inject_neutral: forall (thr: list block) (m: mem), Prop.

Axiom neutral_inject:
  forall m, inject_neutral (valid_block_list m) m ->
  inject (flat_inj (valid_block_list m)) m m.

(* [ REMOVED ] *)
(* we do not define empty memory *)
(* 
