...
forall (OK_spec: ext_spec OK_ty) x,
...
| semax_func_nil:
    forall C V G ge, semax_func V G (C:=C) ge nil nil
| semax_func_cons:
...
| semax_func_app:
  forall cs ge V H funs1 funs2 G1 G2
         (SF1: semax_func V H (C:=cs) ge funs1 G1) (SF2: semax_func V H ge funs2 G2)
...
| semax_func_subsumption:
  ...
  forall funs G (SF: semax_func V F (C:=cs) ge funs G),  semax_func V' F' ge funs G
                                                                       
| semax_func_join:
  forall {cs ge V1 H1 V2 H2 V funs1 funs2 G1 G2 H}
  (SF1: semax_func V1 H1 (C:=cs) ge funs1 G1) (SF2: semax_func V2 H2 ge funs2 G2)
...
Proof.
  ...
  + subst c.
    ...
    iApply oboxopt_ENTAILL;[done | | by iFrame "#"].
    apply wand_ENTAILL; [reduceLL; apply ENTAIL_refl |].
    apply derives_full_fupd_left, H1.
  + iIntros "H".
    iMod (fupd_mask_subseteq E') as "Hmask"; first done.
  ...
  + subst c.
    ...
    - apply later_ENTAILL.
      ...
      * destruct (is_neutral_cast (implicit_deref (typeof e)) t) eqn:Ht; [|normalize; iIntros "(_ & _ & _ & [])"].
        ...
        iIntros "(% & _ & H & _)".
        iPoseProof (typecheck_expr_sound with "H") as "%"; try done. iPureIntro.
    ...
    - apply exp_ENTAILL; intro sh.
      ...
      * iIntros "(? & ? & >?)"; iApply H1; iFrame.
  + iIntros "H".
    iMod (fupd_mask_subseteq E') as "Hmask"; try done.
    iMod (IHsemax with "H") as "H"; try done.
    iMod "Hmask" as "_"; iIntros "!>".
...
Proof.
  ...
  + eapply semax_pre; [| apply AuxDefs.semax_call_backward].
    ...
    iNext; iDestruct "H" as "($ & H)".
    iStopProof; split => rho; monPred.unseal; rewrite monPred_at_affinely; iIntros "(% & ?)".
    iApply oboxopt_sub; auto; try done.
    * eapply tc_fn_return_temp_guard_opt; eauto.
    * eapply semax_lemmas.typecheck_environ_sub; eauto.
...
Lemma tc_expr_NoVundef {CS:compspecs} Delta rho e (TE: typecheck_environ Delta rho):
...
Lemma semax_body_subsumption: forall {CS:compspecs} V V' F F' f spec
