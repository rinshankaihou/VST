...
intros.
iIntros "H".
iDestruct (typecheck_expr_sound with "[H]") as %?;[done | by iDestruct "H" as "(_ & _ & $)"|].
iDestruct (typecheck_expr_sound with "[H]") as %?;[done | by iDestruct "H" as "($ & _)"|].
...
unfold typecheck_expr; fold typecheck_expr.
rewrite denote_tc_assert_andp.
iDestruct (typecheck_expr_sound with "[H]") as %?; try done.
{ iDestruct "H" as "($ & _)". }
iDestruct (proj1 IHe with "[$Hm H]") as %?.
{ iDestruct "H" as "($ & _)". }
iDestruct "H" as "[_ H]"; iDestruct (cop2_sem_cast' with "[$]") as %?; try done; iPureIntro.
...
+ 
  ...
  apply Clight.deref_loc_reference; auto.
*
  iDestruct (typecheck_lvalue_sound with "H") as %TC; try done.
  simpl in TC.
