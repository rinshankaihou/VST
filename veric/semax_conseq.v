...
Proof.
  ...
  + rewrite /bi_affinely; iSplit; first done; iSplit; first done.
    ...
    all: rewrite fupd_mask_mono // in Hx; iIntros "($ & ? & ? & $ & ?)"; rewrite -Hx; iFrame.
...
Proof.
intros.
apply semax'_conseq; [by iIntros "(_ & _ & $)" | .. | intros; rewrite -H0; iIntros "(? & _ & ?)"; iFrame]; intros.
- specialize (H EK_normal None); simpl in H.
  rewrite (bi.pure_True (None = None)) in H; last done; rewrite !bi.True_and in H.
  rewrite -H; last done; iIntros "(? & _ & ?)"; iFrame.
